...
## Overview
This repository contains code to subscripe to the image topic and obtain image data.
...
- Type `ls /dev/video*` and look for the correct webcam device (e.g. /dev/video1).
    ...
    - Also, change the `/dev/video*` on line 92 of the `usb_cam_node.cpp`.
...
# Calibration
Once the launch file is ran, run the following command. If your checkboard has 8 rows and 6 columns, make sure to put 7x5. 
```
rosrun camera_calibration cameracalibrator.py --size 7x5 --square 0.031 image:=/usb_cam/image_raw camera:=/usb_cam
```

Make sure to run before the calibration.
```
...
Git clone the april tag github by running the following command. Also follow the instructions on [here](https://github.com/AprilRobotics/apriltag_ros).

Git clone the april tag github by running the following command. 

```
git clone git@github.com:AprilRobotics/apriltag_ros.git
```
>>>>>>> 09703037c017897596be5f8b21a9d23445773892
